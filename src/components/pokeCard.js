...
function pokeCard(name, image, types, id) {
  return (
    <div key={id} className='pokeCard'>
        ...
        <div className='pokeTipos'>{types.map((t, index) => 
        <div key={index} className={t.type.name}><img className='typeImage' alt='pokeimage' src={typeImageGenerator(t.type.name)}/></div>)}</div>
    </div>
  )
}
