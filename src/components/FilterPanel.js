...
function FilterPanel() {
  ...
  const { genValue, setGenValue, pokeData, setPokeList, pokeType, setPokeType, pokeList} = useAuth();
   ...
   const filtroType = (e) => {
     ...
     if (pokeType[name] === false) {
      if (genValue.g1 === true) {
        ...
        setPokeList([...pokeData.filter((e, i) => i < 151 )].filter((e) => e.types.some((ee) => ee.type.name === name) === true)) }
       if (genValue.g2 === true) {
         setPokeType({ ...typesOff, [name]: !pokeType[name],})
          return setPokeList([...pokeData.filter((e, i) => i > 150 && i < 251)].filter((e) => e.types.some((ee) => ee.type.name === name) === true)); }
         if (genValue.g3 === true) {
          setPokeType({ ...typesOff, [name]: !pokeType[name],})
           return setPokeList([...pokeData.filter((e, i) => i > 250 )].filter((e) => e.types.some((ee) => ee.type.name === name) === true));}
     }
  ...
  return (
    <div className='filterPanel'>
     ...
     <div><img className='pikachuPanel' alt='runningPikachu' src='https://www.pikpng.com/pngl/b/148-1480861_the-pokedex-is-a-digital-encyclopedia-created-by.png'/></div>
